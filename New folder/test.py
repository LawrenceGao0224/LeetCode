...
excel_worksheet = excel_workbook.Sheets('Sheet6')  # Adjust the sheet name as needed
...
for row_index in range(1, excel_worksheet.UsedRange.Rows.Count + 1):
    slide = ppt_presentation.Slides.Add(Index=row_index, Layout=3)  # Layout 1 is Title Slide
    ...
    slide.Shapes[0].TextFrame.TextRange.Text = str(cell_value_B) + " - " + str(cell_value_C) + "\n" + str(int(cell_value_A))+ ". " + str(cell_value_D) + " - " + str(cell_value_E)
    slide.Shapes[1].TextFrame.TextRange.Text = str(cell_value_F) 
    #slide.Shapes[2].TextFrame.TextRange.Text = str(cell_value_G)
